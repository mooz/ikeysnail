...
class LocationBarCompletion extends Component {
  constructor(browser, TOPBAR_HEIGHT, CANDIDATE_HEIGHT = 40) {
    super();
  ...
  selectNextCandidate() {
    if (this.state.suggestionIndex < 0) {
      ...
      this.setState({
        suggestionIndex:
          (this.state.suggestionIndex + 1) % this.state.suggestionList.length
      });
    }
  }
  ...
  build() {
    ...
    return {
      ...
      layout: (make, view) => {
        ...
        make.height
          .equalTo(this.CANDIDATE_HEIGHT * (candidates || []).length)
          .priority(1);
        // このコードだと、最初に候補が出たときかならず表示がバグる （ スクリーン上部にはりつく)
        // make.top.equalTo(locationBar.bottom).priority(1);
        // なので、汚いがピクセル固定でごまかす。。
        const URLBAR_HEIGHT =
          this.TOPBAR_HEIGHT * this._locationBar.HEIGHT_RATIO;
        make.top.equalTo(
          URLBAR_HEIGHT + (this.TOPBAR_HEIGHT - URLBAR_HEIGHT) / 2 + 1
        );
      }
    };
  }
...
// TODO: Or use sqlite!
function findPageEntriesByQuery(
  query,
  urls,
  titles,
  reverse = false,
...
class Suggestion {
  ...
  get icon() {
    return $icon(this.iconType, $rgba(140, 140, 140, 0.8), $size(13, 13));
  }
}
...
class SuggestionTab extends Suggestion {
  ...
  static async generateByQuery(query, browser) {
    ...
    const result = await findPageEntriesByQuery(
      query,
      urls,
      titles,
      false,
      ...
      $http.request({
        ...
        handler: function(resp) {
          if (resp.error) {
            ...
            resolve(
              words.map(word => {
                word = word.replace(/[\\|¥]u([\d\w]{4})/gi, (match, grp) =>
                  String.fromCharCode(parseInt(grp, 16))
                );
                return new SuggestionWebQuery(word, "Google");
              })
            );
          }
        }
        ...
        handler: function(resp) {
          if (resp.error || resp.data.statusCode === 401) {
            ...
            resolve(
              resp.data.pages.map(
                p =>
                  new SuggestionScrapbox(
                    p.title,
                    `https://scrapbox.io/${userName}/${encodeURIComponent(
                      p.title
                    )}`
                  )
              )
