...
function toJSBoxKey(compositeKeyString) {
    let jsBoxKeyObject = compositeKeyString.split("-").reduce((key, keyString) => {
...
function toJSKey(compositeKeyString) {
    return compositeKeyString.split("-").reduce((key, keyString) => {
        keyString = keyString.toUpperCase();
        ...
        return key;
    }, {key: null, ctrlKey: false, shiftKey: false, altKey: false, metaKey: false, press: false});
}


function keyStringToDispatchScript(toKeys) {
    return toKeys.map(compositeKeyString => {
        const key = toJSKey(compositeKeyString);
        if (key.press) {
            return `jsbox.dispatchKeypress(${key.key}, ${key.shiftKey}, ${key.ctrlKey}, ${key.altKey}, ${key.metaKey});`;
        } else {
            return `jsbox.dispatchKeydown(${key.key}, ${key.shiftKey}, ${key.ctrlKey}, ${key.altKey}, ${key.metaKey});`;
        }
    }).join("\n");
}

function dispatchKeys(keys, promisify = false) {
    if ((typeof keys) === "string") {
        keys = [keys];
    }
    const contentScript = keyStringToDispatchScript(keys);
